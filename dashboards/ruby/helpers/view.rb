...
# View helpers — badges, formatting, flash messages
module Sinatra
  module ViewHelper
    def active?(path)
    ...
    def badge(text, variant = 'default')
      "<span class=\"badge badge-#{escape_html(variant)}\">#{escape_html(text.to_s)}</span>"
    end
    ...
    def direction_badge(direction)
      v = case direction.to_s.downcase
          when 'inbound'  then 'primary'
          when 'outbound' then 'success'
          when 'internal' then 'default'
          else 'default'
          end
      badge(direction.to_s.empty? ? '-' : direction, v)
    end
    ...
    def channel_badge(channel)
      ...
      badge(channel.to_s.empty? ? '-' : channel, v)
    end
    ...
    def flash_messages
      ...
      "<div class=\"flash flash-#{escape_html(type)}\">#{escape_html(msg)}</div>"
    end
