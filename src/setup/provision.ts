...
async function deploy(
  ...
  if (deployTarget === 'cloud' && cloud) {
    ...
    try {
      ...
      // Find full path to cloudflared (needed on Windows where it may not be in system PATH)
      // Find cloudflared full path
      let cfPath = '';
      try {
        const whichBin = process.platform === 'win32' ? 'where' : 'which';
        cfPath = execSync(`${whichBin} cloudflared`, { encoding: 'utf8', timeout: 5000 }).trim().split('\n')[0].trim();
      } catch {}
      // Also check common Windows install location
      if (!cfPath && process.platform === 'win32') {
        ...
        try { statSync(candidate); cfPath = candidate; } catch {}
      }
      if (!cfPath) throw new Error('cloudflared not found — install it first');

      // Write CJS wrapper scripts for PM2 (PM2 on Windows can't spawn .cmd files)
      const amDir = join(homedir(), '.agenticmail');
      const { writeFileSync, mkdirSync } = await import('fs');
      try { mkdirSync(amDir, { recursive: true }); } catch {}

      // Enterprise start wrapper — reads env from .env file.
      //
      // # Why this script doesn't just say `npx @agenticmail/enterprise start`
      //
      // npx caches packages indefinitely in `%LOCALAPPDATA%\npm-cache\_npx\<hash>`
      // on Windows (and `~/.npm/_npx/<hash>` on Unix). When PM2 restarts this
      // wrapper, `npx @agenticmail/enterprise start` reuses whatever's already
      // in that cache dir — even if the operator just ran `npm install -g
      // @agenticmail/enterprise@latest`. The result is a stuck-on-old-version
      // bug that reproduces on every Windows box: operator updates globally,
      // PM2 restart, version doesn't change, operator confused.
      //
      // The fix is to skip npx entirely when there's a globally-installed bin
      // and invoke that script directly with `node`. Falls back to `npx -y
      // @agenticmail/enterprise@latest` only for fresh boxes where the
      // operator hasn't done a global install yet (and `-y --latest` forces a
      // registry fetch instead of cache-reuse).
      const startScript = join(amDir, 'start.cjs');
      writeFileSync(startScript, [
        ...
        `    // ALWAYS overwrite from .env — don't skip when process.env[key] is`,
        `    // already set. PM2's saved dump.pm2 can contain stale env captured`,
        `    // before a config change (e.g. an old DATABASE_URL from before a DB`,
        `    // migration). If we honor that stale env over the on-disk .env, the`,
        `    // app silently keeps using the old config even though the operator`,
        `    // updated their .env file. .env on disk is the canonical source.`,
        `    if (m) process.env[m[1]] = m[2];`,
        `  }`,
        `} catch {}`,
        ``,
        `// Resolve the globally-installed bin so PM2 restarts pick up the`,
        `// version the operator installed via \`npm install -g\`. Skipping`,
        `// npx avoids its infinite cache reuse on Windows.`,
        `function findGlobalBin() {`,
        `  try {`,
        `    const prefix = execSync('npm prefix -g', { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();`,
        `    // npm puts global packages in different places per platform:`,
        `    //   Windows: <prefix>\\\\node_modules\\\\<scope>\\\\<pkg>`,
        `    //   Unix:    <prefix>/lib/node_modules/<scope>/<pkg>`,
        `    const candidates = process.platform === 'win32'`,
        `      ? [join(prefix, 'node_modules', '@agenticmail', 'enterprise', 'bin', 'agenticmail-enterprise.cjs')]`,
        `      : [join(prefix, 'lib', 'node_modules', '@agenticmail', 'enterprise', 'bin', 'agenticmail-enterprise.cjs')];`,
        ...
        `let r;`,
        `// windowsHide: when PM2 daemon was started from a console, child`,
        `// processes inherit a window handle. windowsHide makes the spawned`,
        `// node process detach from that handle so no cmd window shows.`,
        `if (globalBin) {`,
        `  // Direct node invocation — fastest, no npx involved, picks up`,
        `  // every \`npm install -g\` immediately on the next PM2 restart.`,
        `  r = spawnSync(process.execPath, [globalBin, 'start'], {`,
        `    stdio: 'inherit', env: process.env, windowsHide: true,`,
        `  });`,
        `} else {`,
        `  // Fallback path for fresh boxes that only ran \`npx ... setup\``,
        `  // and never did a global install. \`-y --latest\` forces a`,
        `  // registry check instead of cache reuse, so updates still land.`,
        `  r = spawnSync('npx', ['-y', '@agenticmail/enterprise@latest', 'start'], {`,
        `    stdio: 'inherit', env: process.env, shell: process.platform === 'win32', windowsHide: true,`,
        `  });`,
        `}`,
      ...
      // Cloudflared wrapper
      const cfScript = join(amDir, 'cloudflared.cjs');
      writeFileSync(cfScript, [
        `// Auto-generated by AgenticMail setup — do not edit`,
      ...
      // Append to existing .env if needed (env file was already written earlier)
      const envFile = join(amDir, '.env');
      try {
      ...
      // Start enterprise via PM2
      execSync(`pm2 start "${startScript}" --name enterprise`, {
      ...
      // Wait then verify processes are running
      await new Promise(r => setTimeout(r, 5000));
      const jlist = execSync('pm2 jlist', { encoding: 'utf8', timeout: 10000, windowsHide: true });
      const procs = JSON.parse(jlist);
      const cfOnline = procs.find((p: any) => p.name === 'cloudflared')?.pm2_env?.status === 'online';
      const entOnline = procs.find((p: any) => p.name === 'enterprise')?.pm2_env?.status === 'online';
      ...
      // Fallback: start cloudflared + server as detached child processes
      let fallbackCfPath = '';
      try {
        const whichBin = process.platform === 'win32' ? 'where' : 'which';
        fallbackCfPath = execSync(`${whichBin} cloudflared`, { encoding: 'utf8', timeout: 5000 }).trim().split('\n')[0].trim();
      } catch {}
      if (!fallbackCfPath && process.platform === 'win32') {
        const localAppData = process.env.LOCALAPPDATA || `${process.env.USERPROFILE}\\AppData\\Local`;
        const candidate = `${localAppData}\\cloudflared\\cloudflared.exe`;
        try { statSync(candidate); fallbackCfPath = candidate; } catch {}
      }
      ...
      // Start enterprise server via node + spawn with shell on Windows
      const entProc = spawn('npx', ['@agenticmail/enterprise', 'start'], {
        detached: true, stdio: 'ignore', env: serverEnv,
