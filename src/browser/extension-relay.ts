...
const RELAY_AUTH_HEADER = "x-agenticmail-relay-token";
...
function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.AGENTICMAIL_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
async function looksLikeAgenticMailRelay(baseUrl: string): Promise<boolean> {
