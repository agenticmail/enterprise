  ...
  if (options?.emailConfig?.smtpHost && (!tp || smtpProvider === 'imap' || smtpProvider === 'smtp')) {
    ...
    try {
      ...
      var smtpToolDefs: Record<string, { desc: string; params: any }> = {
        email_send: { desc: 'Send an email via SMTP. For rich HTML with images, you do NOT need to host images externally: just put `data:` base64 images in the HTML and they are auto-embedded as inline (cid) attachments — the recipient sees them inline, no external hosting. For LARGE HTML (big/many images), write the HTML to a file with the `write` tool and pass `htmlPath` instead of `html` to avoid the argument-size limit. You can also attach files by path via `attachments`.', params: { type: 'object', properties: { to: { type: 'string', description: 'Recipient email' }, cc: { type: 'string' }, bcc: { type: 'string' }, subject: { type: 'string' }, body: { type: 'string', description: 'Plain-text body' }, html: { type: 'string', description: 'HTML body. data: base64 images are auto-converted to inline cid attachments. For large HTML use htmlPath instead.' }, htmlPath: { type: 'string', description: 'Path to a file containing the HTML body (use this for large emails — bypasses the argument size limit). Takes precedence over html.' }, textPath: { type: 'string', description: 'Path to a file containing the plain-text body.' }, attachments: { type: 'array', description: 'File attachments. Each: { path } (read from disk, preferred) OR { content, encoding:"base64" }, plus optional { filename, contentType, cid }. A cid makes it an inline image referenced by <img src="cid:THE_CID">.', items: { type: 'object', properties: { path: { type: 'string' }, content: { type: 'string' }, encoding: { type: 'string', enum: ['base64', 'utf-8'] }, filename: { type: 'string' }, contentType: { type: 'string' }, cid: { type: 'string' } } } }, inlineImages: { type: 'array', description: 'Inline images embedded in the message (multipart/related). Each: { path|content, cid, filename, contentType }. Reference in HTML as <img src="cid:THE_CID">. Auto-cid if omitted.', items: { type: 'object', properties: { path: { type: 'string' }, content: { type: 'string' }, encoding: { type: 'string' }, cid: { type: 'string' }, filename: { type: 'string' }, contentType: { type: 'string' } } } }, replyTo: { type: 'string' } }, required: ['to'] } },
        email_reply: { desc: 'Reply to an email by UID. Supports the same rich-HTML/attachments options as email_send (html, htmlPath, attachments, inlineImages, data: auto-embed).', params: { type: 'object', properties: { uid: { type: 'number', description: 'Email UID to reply to' }, folder: { type: 'string' }, body: { type: 'string' }, html: { type: 'string' }, htmlPath: { type: 'string', description: 'Path to HTML body file (for large emails).' }, attachments: { type: 'array', items: { type: 'object' } }, inlineImages: { type: 'array', items: { type: 'object' } }, all: { type: 'boolean', description: 'Reply all' } }, required: ['uid'] } },
        ...
        email_search: { desc: 'Search emails. Defaults to INBOX. To find messages YOU sent, set folder="[Gmail]/Sent Mail" (Gmail) or "Sent" (others). Use email_folders to list available folders.', params: { type: 'object', properties: { query: { type: 'string' }, from: { type: 'string' }, to: { type: 'string' }, subject: { type: 'string' }, since: { type: 'string' }, before: { type: 'string' }, folder: { type: 'string', description: 'Default INBOX. For your own sent messages use "[Gmail]/Sent Mail".' }, limit: { type: 'number' } } } },
        email_read: { desc: 'Read a specific email by UID. Defaults to INBOX — for sent messages set folder="[Gmail]/Sent Mail".', params: { type: 'object', properties: { uid: { type: 'number' }, folder: { type: 'string', description: 'Default INBOX. Sent messages live in "[Gmail]/Sent Mail".' }, markRead: { type: 'boolean' } }, required: ['uid'] } },
        email_list: { desc: 'List recent emails. Defaults to INBOX — pass folder="[Gmail]/Sent Mail" to list your own sent messages.', params: { type: 'object', properties: { folder: { type: 'string', description: 'Default INBOX. Sent messages live in "[Gmail]/Sent Mail".' }, limit: { type: 'number' }, unreadOnly: { type: 'boolean' } } } },
